...
class PmsReservationService(Component):
    ...
    def create_reservation_checkin_partner(
        ...
        if any(
            ...
            raise MissingError(
                _(
                    "This reservation has %s adults and %s children"
                    " and already has %s checkins created"
                )
                % (
                    reservation_rec.adults,
                    reservation_rec.children,
                    len(reservation_rec.checkin_partner_ids),
                )
            )
