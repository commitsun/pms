...
class PmsPartnerService(Component):
    ...
    def get_partner_address(self, partner_id):
        partner = self.env["res.partner"].browse(partner_id)
        other_contact_ids = False
        if partner:
            other_contact_ids = partner.other_contact_ids
        result_res_zip = []
        ResCityZipInfo = self.env.datamodels["res.city.zip.info"]
        if partner.residence_country_id.id == partner.id_numbers[0].country_id.id:
            result_res_zip.append(
                ResCityZipInfo(
                    street=partner.residence_street
                    if partner.residence_street
                    else None,
                    cityId=partner.residence_city if partner.residence_city else None,
                    stateId=partner.residence_state_id.id
                    if partner.residence_state_id
                    else None,
                    stateName=partner.residence_state_id.name
                    if partner.residence_state_id
                    else None,
                    countryId=partner.residence_country_id.id
                    if partner.residence_country_id
                    else None,
                    zipCode=partner.residence_zip if partner.residence_zip else None,
                    documentNumber=partner.id_numbers[0].name
                    if partner.id_numbers
                    else None,
                )
            )
        for address in other_contact_ids:
            result_res_zip.append(
                ResCityZipInfo(
                    street=address.street if address.street else None,
                    cityId=address.city if address.city else None,
                    ...
                    zipCode=address.zip if address.zip else None,
                    documentNumber=address.id_numbers[0].name
                    if address.id_numbers
                    else None,
                )
            )
