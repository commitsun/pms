...
# @tagged("test_debug")
class TestRR(common.TestWubookConnector):
    def test_rr(self):
        import json
        ...
        # print(a)
        #
        # def convert_format(elem, mapper, path=""):
        #     if isinstance(elem, dict):
        #         for k, v in elem.items():
        #             current_path = "%s/%s" % (path, k)
        #             if isinstance(v, (tuple, list, dict)):
        #                 convert_format(v, mapper, current_path)
        #             elif isinstance(v, (str, int, float, bool)):
        #                 if current_path in mapper:
        #                     elem[k] = mapper[current_path](elem[k])
        #             else:
        #                 raise NotImplementedError("Type %s not implemented" % type(v))
        #     elif isinstance(elem, (tuple, list)):
        #         for ch in elem:
        #             convert_format(ch, mapper, path)
        ...
        #     '/booked_rooms/roomdays/day': lambda x: x + "XXXXXXX",  # datetime.datetime.strptime(x, "%d/%m/%Y")
        # }
        # convert_format(a, mc)
        # with open("wubook_reservation_mod.json", 'w') as f:
        #     json.dump(a, f)

        with backend.work_on("channel.wubook.pms.reservation") as work:
        ...
        for r in res:
            ...
            for k, v in r.items():
                print("   {}: {}".format(k, v))
        print("-----------", len(res))
        with open("wubook_reservation_disc.json", "w") as f:
            json.dump(res, f)
