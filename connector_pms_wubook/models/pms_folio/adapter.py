...
class ChannelWubookPmsFolioAdapter(Component):
    ...
    def read(self, _id, ancillary=True):
        ...
        value = self._exec("fetch_booking", *params)
        if not value:
            return False
        return value[0]
    ...
    def search_read(self, domain, ancillary=True, mark=False, only_codes=False):
        ...
        * fetch_booking(token, lcode, rcode[, ancillary= False])
        ...
        if domain_date:
            ...
            date_field_args = [("%s_%%s" % date_field) % x for x in ["from", "dto"]]
            if only_codes:
        ...
        conv_mapper = {
            ...
            "/roomnight": lambda x: None,  # useless
            "/booked_rate": lambda x: None,  # deprecated
            "/date_received": lambda x: None,  # deprecated
            "/deleted_at": lambda x: None,  # deprecated
        }
        ...
        for value in values:
            ...
            value["rate_id"] = None
            rate_ids = set()
            for room in value["booked_rooms"]:
                rate_ids |= {x["rate_id"] for x in room["roomdays"]}
            if len(rate_ids) == 1:
                value["rate_id"] = rate_ids.pop()
            ...
            boards_d = {
                k: v != "nb" and v or None for k, v in value.pop("boards").items()
            }
            ...
            for room in value.pop("booked_rooms"):
                ...
                for days in room["roomdays"]:
                    if room_rate_id is None:
                        room_rate_id = days["rate_id"]
                    else:
                        if room_rate_id != days["rate_id"]:
                            raise ValidationError(
                ...
                reservations.append(
                    {
                        ...
                        "date_arrival": value["date_arrival"],
                        "date_departure": value["date_departure"],
                        "ancillary": room["ancillary"],
                        "board": boards_d[room_id],
